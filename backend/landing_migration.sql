-- =========================================================================
-- SCRIPT DE MIGRACIÓN FINAL Y DEFINITIVO PARA EL LANDING PAGE DE CEPSITCED
-- =========================================================================

-- 1. Eliminar tablas antiguas si existen para evitar conflictos
DROP TABLE IF EXISTS public.landing_configuracion CASCADE;
DROP TABLE IF EXISTS public.landing_nosotros CASCADE;

-- 2. Crear tabla 'landing_configuracion' (Sólo interruptores booleanos)
CREATE TABLE public.landing_configuracion (
  ...
  mostrar_faq boolean DEFAULT true, -- Nueva columna obligatoria
  updated_at timestamp with time zone DEFAULT timezone('utc'::text, now()),
  CONSTRAINT landing_configuracion_pkey PRIMARY KEY (id)
);

-- Habilitar RLS para 'landing_configuracion'
ALTER TABLE public.landing_configuracion ENABLE ROW LEVEL SECURITY;

-- Crear políticas de acceso para 'landing_configuracion'
...
-- 3. Crear tabla 'landing_nosotros' (Contenido institucional)
CREATE TABLE public.landing_nosotros (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  mision_subtitulo character varying(255) DEFAULT 'Comprometidos con tu bienestar emocional',
  mision_texto text DEFAULT 'Ofrecer y brindar servicios psicológicos dirigidos a estudiantes, instituciones y comunidades, promoviendo la salud mental en nuestra sociedad a través de una atención responsable, ética y comprometida con el bienestar de los pacientes en todas las áreas.',
  vision_subtitulo character varying(255) DEFAULT 'Líderes en salud mental y desarrollo humano',
  vision_texto text DEFAULT 'Ser una institución destacada y reconocida en el ámbito de la salud mental y el apoyo a instituciones educativas, contribuyendo a la prevención, promoción e intervención psicológica con calidad humana, ética profesional y compromiso social.',
  imagen_url text DEFAULT 'https://placehold.co/600x400/007bff/ffffff?text=CEPSITCED+Nosotros', -- Nueva columna obligatoria
  creado_at timestamp with time zone DEFAULT timezone('utc'::text, now()),
  updated_at timestamp with time zone DEFAULT timezone('utc'::text, now()),
  CONSTRAINT landing_nosotros_pkey PRIMARY KEY (id)
);

-- Habilitar RLS para 'landing_nosotros'
ALTER TABLE public.landing_nosotros ENABLE ROW LEVEL SECURITY;

-- Crear políticas de acceso para 'landing_nosotros'
...
-- 4. Insertar datos semilla iniciales
