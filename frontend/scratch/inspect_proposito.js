...
async function test() {
  const { data, error } = await supabase.from('landing_carousel').select('*');
  if (error) {
    console.log('Error querying landing_carousel:', error.message);
  } else {
    console.log('All landing_carousel records:', data);
  }
}
