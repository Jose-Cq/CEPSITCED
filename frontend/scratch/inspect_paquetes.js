...
async function test() {
  const { data, error } = await supabase.from('paquetes_catalogo').select('*').limit(3);
  if (error) {
    console.log('Error querying packages catalog:', error.message);
  } else {
    console.log('Sample catalog packages:', data);
  }
}
